...
#-- I don't put this timestamp in yet, but it should be folded into the netcdf files output like Andy J. does
...
#-- This keeps the seed the same for reproducability.  I need to double check this doesn't adversely effect simulations
#- -The random portion is generated as a vector so we certainly don't have every simulation w/ the same random pull : ))

set.seed(27)

#--Queing software, nasa, torque or sge
#-- Torque was for CIRA, sge was for ATMOS/BIOCYCLE
#--  only NASA currently works (DISCOVER)

que_soft = "nasa"

#-- Propagation Choices are 'none', 'pure' or 'ct'
#-- prop_model ct is 'new prior' = function(last two state estimates) + random, like CT
#-- not used explicitly for this CarbonTracker script
prop_model = "ct"
...
#-- These are somewhat redundant args, just make sure endcycle is AT LEAST larger than your # cycles desired
cycles = 1:(16*52)
startcycle = cycles[1]
endcycle = cycles[length(cycles)]

#-- THis is not used for CT simulation
...
#-- Some machinery for this is still in scripts but generally not used, or to be trusted
estimate_land_ocean_bias = FALSE

#-- This controls the "pods" script to run serial runs on Discover
pods_numb = 24

#-- Command line args, pulls in the args from the runscripts, i.e. cycle, startdate, etc
...
#-- User directories
#-- run_dir is where the executable resides
...
#-- Where output goes, in that folder,you need subfolders, betas, CO2, flasks, stations, maybe? more
outdir = "/discover/nobackup/aschuh/GEOS-CHEM_output/v9.2_geosfp_CT_4x5"

#-- The location for the GEOSCHEM namelist
input_geos_file = paste(run_dir,"/input.geos",sep="")
#orig_betas_file = "/discover/nobackup/aschuh/data/betas/betas.061213.nc"

#-- Set working directory, make sure necessary R files are in there
...
#-- This is main OUTER assimilation cycle loop
...
for(i in startcycle:endcycle)
{
    ...
    #-- OF SCREWED UP IN STARTDATE AND RUNDATE
       ...
       if(que_soft=="nasa"){
         ...
         #-- I used this point to stop the run after the first ensembles are created.  This allows me to interactively step through the optimization 
         #-- code below to see what is occurring and/or debug the optim.
	   ...
	   print("merging data...")
        ...
        #-- This is MY code to merge the GCHEM output together in order to run Andy's code 
        ...
        flasks.in <- sapply(t.ins,FUN=function(x){paste(outdir,"/../../run/prepped_daily_latest_obspack/flask_input.",x,".nc",sep="")})
 ...
 #-- Load data
   ...
   ensnum = sapply(fls.short,
    ...
    return(out)
   } #--  end of merge_ens_obspack_data


   #-- Now pull the data into 'out' object
   ...
   #-  Main observation loop, this is Andy's code below here
   #-- This is pretty vanilla from W&H2002, I've rewritten this as 
   #-- dynamic fortran library using OPENMP for future (fyi).
...
}    #*******  THIS IS END OF THE OPTIMIZATION LOOP  ********
#progress.bar.end(pb)


   #-- This is organizing the output and writing it out
...
#covariance.prior <- make.covariance(nparms,bio.cov,ocean.nc.dir,deltapco2.prefix,t0)
	...
	print("launching mean beta rerun to generate new CO2 field ....")
      ...
      {
       Sys.sleep(30)
       restartfilename = paste(outdir,"/CO2/restart.",rdate_arg2,"00.nc",sep="")
       if(file.exists(restartfilename))
          {
            print(paste("Found",restartfilename,", moving to new iteration..."))
            found=TRUE
          }else{
       print(paste("Couldn't find",restartfilename,", waiting 30 secs ...."))
       timer = timer + 1
       if(timer > 6){stop("waited 3 minutes for restart file, quitting....")}
          }
       }
